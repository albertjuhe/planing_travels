 ...
 * @param {Function} onPresenceUpdate  - Called with { count, usernames } when room presence changes
 * @returns {WebSocket}                - The open connection; call ws.close() on unmount
 */
export const connectToTravelRoom = (travelId, onLocationAdded, onPresenceUpdate) => {
    ...
    ws.onmessage = (event) => {
        try {
            ...
            } else if (msg.event === 'presence' && onPresenceUpdate) {
                onPresenceUpdate({ count: msg.count, usernames: msg.usernames });
            }
        } catch (e) {
